...
from webproject.models import Users, PasswordReset
...
def login_post():
    ...
    curr_usr = Users.query.filter_by(email=email).first()
...
def register_post():
    ...
    curr_user = Users.query.filter_by(email=request.form.get('email')).first()
    if curr_user:
        flash('email exists already')
        return redirect(url_for('auth.register'))
    ...
    record = {
        ...
        'role'  : 'admin' if email=='gypsaman@gmail.com' else 'user'
    }
    
    new_usr = Users(**record)
...
def password_reset():
    if request.method == "POST":
        ...
        email = UBEmail()
        body = f'Your password reset link is http://distributedneuralapplications.com/passwordupdate/{password_phrase}'
...
def password_update(id):
    ...
    user = Users.query.filter_by(id=pwdreset.user_id).first()
